    ...
    return (
        <div className="fixed z-50 left-0 bottom-0 w-full bg-light border-t border-t-slate-100 md:hidden shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
            <div className="grid grid-cols-3 items-center">
                {
                    navItems?.map(item =>
                        <Link key={item?.id} href={item?.href || ""} className={` py-3 flex justify-center items-center gap-1 uppercase`}>
                            {item?.icon}
                            <div className="relative text-xs">
                                ...
                                {
                                    item?.title == "Cart" &&
                                    <p className="absolute -top-1 -right-[18px] w-[15px] h-[15px] bg-dark rounded-full text-[9px] text-light flex justify-center">2</p>
                                }
                            </div>
                        </Link>
                    )
